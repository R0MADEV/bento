...
| Tareas (worktrees), **solo lectura** | Responder "¿en qué anda cada rama?" desde fuera. Por HTTP va la lista; por el socket IPC va además el estado, el diff y el historial (`tasks.status`, `tasks.diff`, `tasks.log`, `tasks.upstream`, `tasks.fixup`), que el CLI usa. |
