...
/// Arma una entrada nueva con id y marcas de tiempo, y la valida antes de
/// devolverla. Lo usa quien no tiene un panel delante (el CLI, el daemon):
/// el panel arma la entrada él mismo porque ya la tiene a medio escribir.
pub fn new_entry(
    project_path: &str,
    kind: &str,
    title: &str,
...
/// El identificador de la app de escritorio: es el que decide dónde guarda
/// Tauri los datos, y por tanto dónde vive `memory.sqlite3`.
const APP_IDENTIFIER: &str = "com.romadev.bento";

/// El directorio de datos de la app. El desktop se lo pide a Tauri; el daemon
/// y el CLI lo resuelven aquí para leer y escribir la misma base de datos.
pub fn data_dir() -> Result<std::path::PathBuf, String> {
    ...
    let base = if cfg!(target_os = "macos") {
        ...
        std::env::var("APPDATA").map(std::path::PathBuf::from).unwrap_or(home)
    } else {
...
pub(crate) const MAX_TEXT_LENGTH: usize = 20_000;
...
pub fn memory_list(data_dir: &Path, project_path: String) -> Result<Vec<MemoryEntry>, String> {
...
pub fn memory_list_all(data_dir: &Path) -> Result<Vec<MemoryEntry>, String> {
    let conn = connection(data_dir)?;
    let mut stmt = conn.prepare(
        "SELECT id, project_path, kind, title, summary, details, tags_json, files_json, source, external_id, created_at, updated_at
...
pub fn memory_create(data_dir: &Path, entry: MemoryEntry) -> Result<MemoryEntry, String> {
    let conn = connection(data_dir)?;
...
pub fn memory_remove(data_dir: &Path, project_path: String, id: String) -> Result<bool, String> {
    let conn = connection(data_dir)?;
    Ok(conn
        .execute(
...
pub fn memory_migrate(data_dir: &Path, entries: Vec<MemoryEntry>) -> Result<usize, String> {
    let mut conn = connection(data_dir)?;
...
mod tests {
    ...
    #[test]
    fn a_new_entry_is_valid_and_stamped() {
        let entry = new_entry("/proj", "note", "título", "resumen", "", "cli").unwrap();
        assert!(!entry.id.is_empty());
        assert_eq!(entry.created_at, entry.updated_at);
        assert_eq!(entry.project_path, "/proj");
        assert!(validate::validate_entry(&entry).is_ok());
    }
