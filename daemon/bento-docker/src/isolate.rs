...
/// Genera un `docker-compose.override.yml` en la tarea que reasigna la subred,
/// los nombres de contenedor y los puertos, para que su stack pueda correr a la
/// vez que el del repo principal sin chocar.
///
/// Los puertos salen de: 20000 + offset_de_subred×100 + último octeto de la IP.
/// Los expuestos se descubren inspeccionando los contenedores del stack madre.
///
/// Devuelve el error "no-compose" si no hay docker-compose.yml, que el llamante
/// trata como "aquí no hay nada que aislar".
pub fn isolate(worktree_path: &str) -> Result<IsolateResult, String> {
