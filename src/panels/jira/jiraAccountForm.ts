...
export function buildJiraAccountForm(deps: JiraAccountFormDeps): (existing?: JiraAccount) => void {
  const showConfig = (existing?: JiraAccount): void => {
    const siteF = field(i18nT('jira.sitePlaceholder'), existing?.site ?? '')
    const emailF = field(i18nT('jira.email'), existing?.email ?? '')
    const tokenF = field(i18nT('jira.apiToken'), existing?.token ?? '', 'password')
    ...
    save.addEventListener('click', async () => {
      ...
      if (!s || !e || !t) { status.textContent = i18nT('jira.allFieldsAreRequired'); return }
      try {
